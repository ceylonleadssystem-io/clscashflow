(function() {
  ...
  function normalizeInvoiceSettings(settings) {
    ...
    return {
      ...
      email: email,
      vat: settings.vat || '',
      registrationNumber: settings.registrationNumber || settings.businessRegistrationNumber || settings.brn || '',
  ...
  function clsLegacyInvoiceRendererRemoved(opts) {
    ...
    return '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>' + invoiceEscape(title) + '</title><style>' +
      ...
      '<section class="invoice-head"><div class="brand">' + logo + '<div><div class="biz-name">' + invoiceEscape(s.biz) + '</div><div class="small">' + (s.addr ? invoiceBreaks(s.addr) : '') + (s.email ? (s.addr ? '<br>' : '') + invoiceEscape(s.email) : '') + (s.registrationNumber ? ((s.addr || s.email) ? '<br>' : '') + 'Business registration: ' + invoiceEscape(s.registrationNumber) : '') + (s.vat ? ((s.addr || s.email || s.registrationNumber) ? '<br>' : '') + 'VAT: ' + invoiceEscape(s.vat) : '') + '</div></div></div><div class="title"><h1>Invoice</h1><div class="num">' + invoiceEscape(title) + '</div><div class="meta"><div>Date: ' + humanDate(inv.date) + '</div><div>Due: ' + humanDate(inv.due) + '</div><div>Terms: ' + invoiceEscape(inv.terms || 'Net 30') + '</div></div></div></section>' +
  ...
  window.clsEnsurePaymentRequest = async function clsEnsurePaymentRequest(profile, opts) {
    ...
    var details = PLAN_DETAILS[plan] || PLAN_DETAILS.solo;
    var cycle = billingCycleFor(profile, opts.billingCycle);
    var chargeAmount = cycle === 'annual' ? details.price : details.monthlyPrice;
    var selectedPayLink = planPaymentLink(details, cycle) || '';
    ...
    var request = {
      ...
      monthlyPayLink: details.monthlyPayLink || '',
      annualPayLink: details.annualPayLink || '',
      defaultMonthlyPayLink: details.monthlyPayLink || '',
      defaultAnnualPayLink: details.annualPayLink || '',
	      paymentLink: selectedPayLink,
    ...
    var userUpdate = {
      ...
      paymentRequestAmount: chargeAmount || 0,
      paymentRequestMonthlyAmount: details.monthlyPrice || 0,
      paymentRequestAnnualAmount: details.price,
      paymentProvider: 'bank-transfer',
      paymentMonthlyPayLink: details.monthlyPayLink || '',
      paymentAnnualPayLink: details.annualPayLink || '',
	      paymentLink: selectedPayLink,
	      paymentLinkCycle: cycle,
      billingCycle: cycle,
    ...
    try {
      ...
      profile.paymentRequestAmount = chargeAmount || 0;
      return request;
    } catch (e) {
      console.warn('Payment request token could not be saved:', e);
      return { token: token, plan: plan, planName: details.name, amount: chargeAmount || 0, billingCycle: cycle, unsaved: true };
    }
  };
  ...
  function billingCycleFor(profile,requested) {
    var value=String(requested||(profile&&profile.billingCycle)||'monthly').toLowerCase();
    return value.indexOf('annual')===0||value.indexOf('year')===0?'annual':'monthly';
  }
  ...
  function nextAnnualDueDate(fromDate) {
    var from=fromDate instanceof Date?new Date(fromDate.getTime()):new Date(),next=new Date(from.getTime());
  ...
  function paymentTimelineHtml(profile,requestedCycle) {
    ...
    return '<div class="cls-payment-timeline"><strong>'+(paid?'Current prepaid '+cycle+' cycle':'Trial to prepaid '+cycle+' plan')+'</strong><div><i></i><span><b>'+fmt(reminder)+'</b> · '+(paid?(annual?'Annual renewal reminder':'Monthly payment reminder'):'Trial ending reminder')+'</span></div><div><i></i><span><b>'+fmt(due)+'</b> · '+(paid?'Next '+unit+' must be paid in advance':'Trial ends · first '+cycle+' payment is due')+'</span></div><div><i></i><span><b>After payment</b> · Contact Ceylonry accounts; an administrator confirms payment and activates the next '+unit+'.</span></div></div>';
  }
  function billingCyclePickerHtml(details,selectedCycle) {
    details=details||PLAN_DETAILS.solo;selectedCycle=selectedCycle==='annual'?'annual':'monthly';
    var annualSaving=Math.max(0,(Number(details.monthlyPrice)||0)*12-(Number(details.price)||0)),annualMonthly=(Number(details.price)||0)/12;
    return '<div class="cls-payment-choice" role="group" aria-label="Choose a payment plan"><div class="cls-payment-choice-title">Choose your payment plan</div><div class="cls-payment-choice-grid"><button type="button" data-payment-cycle="monthly" class="'+(selectedCycle==='monthly'?'active':'')+'" aria-pressed="'+(selectedCycle==='monthly')+'"><span>Monthly</span><strong>'+escapeHtml(money(details.monthlyPrice))+'</strong><small>paid each month</small></button><button type="button" data-payment-cycle="annual" class="'+(selectedCycle==='annual'?'active':'')+'" aria-pressed="'+(selectedCycle==='annual')+'"><span>Annual'+(annualSaving?' · Save '+escapeHtml(money(annualSaving)):'')+'</span><strong>'+escapeHtml(money(details.price))+'</strong><small>'+escapeHtml(money(annualMonthly))+'/month, paid yearly</small></button></div><div class="cls-payment-choice-summary" aria-live="polite"></div></div>';
  }
  function bindBillingCyclePicker(root,profile,details,initialCycle) {
    var cycle=initialCycle==='annual'?'annual':'monthly';
    function select(nextCycle) {
      cycle=nextCycle==='annual'?'annual':'monthly';var annual=cycle==='annual',amount=annual?details.price:details.monthlyPrice,unit=annual?'year':'month';
      root.querySelectorAll('[data-payment-cycle]').forEach(function(button){var active=button.getAttribute('data-payment-cycle')===cycle;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active));});
      var kicker=root.querySelector('.cls-billing-kicker'),title=root.querySelector('h2'),copy=root.querySelector('.cls-payment-copy'),timeline=root.querySelector('.cls-payment-timeline'),submit=root.querySelector('.cls-bank-submit'),summary=root.querySelector('.cls-payment-choice-summary'),paidBefore=root.getAttribute('data-paid-before')==='true';
      if(kicker)kicker.textContent='Prepaid '+cycle+' subscription';
      if(title)title.textContent=root.id==='cls-paywall'?(paidBefore?'Pay the next '+unit+' to continue':'Start your prepaid '+cycle+' plan'):'Pay your next '+unit+' in advance';
      if(copy)copy.innerHTML=root.id==='cls-paywall'?(paidBefore?'Your current paid '+unit+' has ended.':'Your free trial has ended.')+' Transfer <b>'+escapeHtml(money(amount))+'</b> in advance for the next '+unit+', then contact Ceylonry accounts for confirmation. Your data remains safe while access is paused.':'Transfer <b>'+escapeHtml(money(amount))+'</b> for your '+escapeHtml(details.name)+' plan, then contact Ceylonry accounts. No payment-slip upload is required.';
      if(timeline)timeline.outerHTML=paymentTimelineHtml(profile,cycle);if(submit)submit.textContent='Contact accounts about '+cycle+' payment';if(summary)summary.innerHTML='Selected: <b>'+cycle.charAt(0).toUpperCase()+cycle.slice(1)+'</b> · Transfer <b>'+escapeHtml(money(amount))+'</b>.';
    }
    root.querySelectorAll('[data-payment-cycle]').forEach(function(button){button.addEventListener('click',function(){select(button.getAttribute('data-payment-cycle'));});});select(cycle);return function(){return cycle;};
  }
  ...
  window.clsSubmitSubscriptionReceipt=async function clsSubmitSubscriptionReceipt(file, profile, plan, statusEl, requestedCycle) {
    profile=profile||window._profile||{};var details=PLAN_DETAILS[plan]||PLAN_DETAILS.solo,user=getAuthUser(),cycle=billingCycleFor(profile,requestedCycle),annual=cycle==='annual',chargeAmount=annual?details.price:details.monthlyPrice;
    ...
    var authToken=user&&user.getIdToken?await user.getIdToken():'';
    var res=await fetch('/.netlify/functions/submit-subscription-receipt',{method:'POST',headers:{'Content-Type':'application/json','Authorization':authToken?'Bearer '+authToken:''},body:JSON.stringify({fileBase64:base64,fileName:file.name,mimeType:file.type,planKey:plan,period:period,billingCycle:cycle})});
  ...
  window.clsOpenBankTransferPayment=function clsOpenBankTransferPayment(plan,profile,requestedCycle){
    injectBillingCardStyle();
    profile=profile||window._profile||{};plan=bestPlan(profile,plan);var details=PLAN_DETAILS[plan]||PLAN_DETAILS.solo,cycle=billingCycleFor(profile,requestedCycle),annual=cycle==='annual',chargeAmount=annual?details.price:details.monthlyPrice,unit=annual?'year':'month',old=document.getElementById('cls-bank-payment-modal');if(old)old.remove();var wrap=document.createElement('div'),email=profileEmail(profile,getAuthUser());wrap.id='cls-bank-payment-modal';wrap.className='cls-bank-modal';wrap.innerHTML='<div class="cls-bank-card"><button class="cls-bank-close" aria-label="Close">×</button><div class="cls-billing-kicker">Prepaid '+cycle+' subscription</div><h2>Pay your next '+unit+' in advance</h2><p class="cls-payment-copy">Transfer <b>'+escapeHtml(money(chargeAmount))+'</b> for your '+escapeHtml(details.name)+' plan. Then contact Ceylonry accounts so an administrator can confirm payment. No payment-slip upload is required.</p>'+billingCyclePickerHtml(details,cycle)+bankDetailsHtml()+paymentTimelineHtml(profile,cycle)+'<button class="cls-bank-submit">Email accounts</button><button class="cls-bank-signout" data-payment-whatsapp>Contact on WhatsApp</button><div class="cls-bank-status">Your account is activated only after an administrator confirms the payment.</div></div>';document.body.appendChild(wrap);bindBillingCyclePicker(wrap,profile,details,cycle);wrap.querySelector('.cls-bank-close').onclick=function(){wrap.remove();};wrap.addEventListener('click',function(e){if(e.target===wrap)wrap.remove();});wrap.querySelector('.cls-bank-submit').onclick=function(){location.href='mailto:'+CLS_BANK.email+'?subject='+encodeURIComponent('Ceylonry POS payment confirmation')+'&body='+encodeURIComponent('Hello, I have paid for my Ceylonry POS account '+email+'. Please confirm the payment and activate my account.')};wrap.querySelector('[data-payment-whatsapp]').onclick=function(){window.clsOpenPlanWhatsApp(plan,profile)};
  };
  window.clsStartCurrentPlanPayment=function(){return window.clsOpenBankTransferPayment(null,window._profile||{});};
  window.clsRenderSubscriptionPaywall=function(profile,opts){opts=opts||{};if(document.getElementById('cls-paywall'))return;injectBillingCardStyle();profile=profile||{};var plan=bestPlan(profile,opts.plan),details=PLAN_DETAILS[plan]||PLAN_DETAILS.solo,cycle=billingCycleFor(profile),annual=cycle==='annual',chargeAmount=annual?details.price:details.monthlyPrice,unit=annual?'year':'month',hasPaidBefore=!!(profile.posPaymentVerifiedAtUtc||profile.nextPaymentDue),ov=document.createElement('div');ov.id='cls-paywall';ov.className='cls-bank-modal';ov.setAttribute('data-paid-before',String(hasPaidBefore));ov.innerHTML='<div class="cls-bank-card cls-bank-paused"><div class="cls-billing-kicker">'+(hasPaidBefore?(annual?'Annual renewal due':'Monthly payment due'):'15-day trial complete')+'</div><h2>'+(hasPaidBefore?'Pay the next '+unit+' to continue':'Start your prepaid '+cycle+' plan')+'</h2><p class="cls-payment-copy">'+(hasPaidBefore?'Your current paid '+unit+' has ended.':'Your free trial has ended.')+' Transfer <b>'+escapeHtml(money(chargeAmount))+'</b> in advance, then contact Ceylonry accounts. No payment-slip upload is required and your data remains safe while access is paused.</p>'+billingCyclePickerHtml(details,cycle)+bankDetailsHtml()+paymentTimelineHtml(profile,cycle)+'<button class="cls-bank-submit">Contact accounts</button><div class="cls-bank-status">An administrator will confirm payment and restore access.</div><button class="cls-bank-signout" onclick="window.clsSignOut&&window.clsSignOut()">Sign out</button></div>';document.body.appendChild(ov);bindBillingCyclePicker(ov,profile,details,cycle);ov.querySelector('.cls-bank-submit').onclick=function(){window.clsOpenBankTransferPayment(plan,profile,cycle)};};
  ...
  function injectBillingCardStyle() {
    ...
    style.textContent =
      ...
      '#cls-billing-widget .cls-cycle-picker{display:inline-flex;gap:4px;padding:4px;margin-top:.9rem;border:1px solid #DED7CC;border-radius:999px;background:#F7F5F0}' +
      '#cls-billing-widget .cls-cycle-picker button{border:0;border-radius:999px;background:transparent;color:#6B6258;padding:.55rem .8rem;font:800 .62rem DM Sans,Inter,Arial,sans-serif;cursor:pointer}' +
      '#cls-billing-widget .cls-cycle-picker button.active{background:#1C1814;color:#fff}' +
      ...
      '.cls-payment-choice{margin:1rem 0}.cls-payment-choice-title{font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.09em;margin-bottom:.55rem}.cls-payment-choice-grid{display:grid;grid-template-columns:1fr 1fr;gap:.65rem}.cls-payment-choice-grid button{display:grid;gap:.2rem;text-align:left;padding:.8rem;border:1px solid #d9d5cc;border-radius:12px;background:#fff;color:#1c1814;cursor:pointer;font-family:inherit}.cls-payment-choice-grid button.active{border:2px solid #1c1814;background:#f7f5f0;padding:calc(.8rem - 1px)}.cls-payment-choice-grid span{font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.05em}.cls-payment-choice-grid strong{font-size:1.05rem}.cls-payment-choice-grid small{font-size:.67rem;color:#68645d}.cls-payment-choice-summary{margin-top:.55rem;padding:.6rem .7rem;border-radius:9px;background:#fff8e8;color:#5e543e;font-size:.74rem}' +
      ...
      '.cls-bank-modal,.cls-bank-modal *{box-sizing:border-box}' +
      '@media(min-width:1180px){#settings-billing-widgets #cls-billing-widget{padding:1.5rem 1.65rem}}' +
      '@media(max-width:760px){#settings-billing-widgets{margin-top:1rem!important}#cls-billing-widget{margin:0 0 1rem;padding:1.1rem}#cls-billing-widget .cls-cycle-picker{display:flex;width:100%}#cls-billing-widget .cls-cycle-picker button{flex:1}#cls-billing-widget .cls-billing-actions{grid-template-columns:1fr;gap:.8rem;margin-top:.2rem}#cls-billing-widget .cls-billing-pay,#cls-billing-widget .cls-billing-monthly,#cls-billing-widget .cls-billing-wa{width:100%}}' +
      '@media(max-width:600px){.cls-bank-modal{place-items:end center;padding:8px}.cls-bank-card{width:100%;max-height:calc(100dvh - 16px);overflow-y:auto;border-radius:18px 18px 10px 10px;padding:1.2rem}.cls-bank-card h2{font-size:1.65rem;padding-right:2.5rem}.cls-bank-card>p{font-size:.82rem}.cls-payment-choice-grid{grid-template-columns:1fr}.cls-bank-details,.cls-payment-timeline{padding:.85rem;margin:.8rem 0}.cls-bank-details>div{align-items:flex-start;gap:.6rem}.cls-bank-details b{overflow-wrap:anywhere}.cls-receipt-upload input{font-size:16px}.cls-bank-submit{min-height:48px;padding:.85rem .7rem;letter-spacing:.06em}.cls-bank-close{position:sticky;top:0;float:right;z-index:2}}' +
	    ...
	    wrap.innerHTML =
	      '<div class="cls-billing-top">' +
	        '<div>' +
	          ...
	          '<div class="cls-cycle-picker" role="group" aria-label="Billing cycle"><button type="button" data-billing-cycle="monthly">Pay monthly</button><button type="button" data-billing-cycle="annual">Pay annually</button></div>' +
	          '<div class="cls-billing-title" data-billing-title></div>' +
	          '<div class="cls-billing-copy" data-billing-copy></div>' +
	          '<div class="cls-billing-price"><span data-billing-price></span><span data-billing-term style="font-size:.9rem;color:#6B6258"></span></div>' +
	          '<div class="cls-billing-sub">' + escapeHtml(periodCopy) + '</div><div class="cls-billing-bank">'+bankDetailsHtml()+'<div data-billing-timeline></div></div>' +
	        '</div>' +
	        '<div class="cls-billing-actions">' + actionHtml + '</div>' +
	      '</div>' +
	      '<div class="cls-billing-status" data-billing-status>After transfer, contact accounts@ceylonrylabs.io or use WhatsApp. An administrator will confirm payment and activate the account.</div>';
	    (document.getElementById('settings-billing-widgets') || settingsView).appendChild(wrap);
	    var submitBtn = wrap.querySelector('[data-billing-submit]');
	    var cycleBtns = wrap.querySelectorAll('[data-billing-cycle]');
	    var requestBtn = wrap.querySelector('[data-billing-request]');
	    var waBtn = wrap.querySelector('[data-billing-wa]');
	    function renderCycle() {
	      var annual = selectedCycle === 'annual';
	      var charge = annual ? details.price : details.monthlyPrice;
	      wrap.querySelector('[data-billing-title]').textContent = annual ? 'Annual bank transfer' : 'Monthly bank transfer';
	      wrap.querySelector('[data-billing-copy]').textContent = 'Pay your CLS ' + details.name + ' ' + selectedCycle + ' package directly to our Commercial Bank account. Contact our accounts team after transfer; no payment-slip upload is required.';
	      wrap.querySelector('[data-billing-price]').textContent = money(charge || 0);
	      wrap.querySelector('[data-billing-term]').textContent = annual ? ' / yr' : ' / mo';
	      wrap.querySelector('[data-billing-timeline]').innerHTML = paymentTimelineHtml(profile, selectedCycle);
	      Array.prototype.forEach.call(cycleBtns, function(btn) { var active=btn.getAttribute('data-billing-cycle')===selectedCycle;btn.classList.toggle('active',active);btn.setAttribute('aria-pressed',active?'true':'false'); });
	      if (submitBtn && !submitBtn.disabled) submitBtn.textContent = annual ? 'Pay annually by bank transfer' : 'Pay monthly by bank transfer';
	    }
	    Array.prototype.forEach.call(cycleBtns, function(btn) { btn.addEventListener('click', function(){ selectedCycle=this.getAttribute('data-billing-cycle')==='annual'?'annual':'monthly';renderCycle(); }); });
	    renderCycle();
	    if (submitBtn&&!submitBtn.disabled) submitBtn.addEventListener('click', function() { window.clsOpenBankTransferPayment(plan,window._profile||profile,selectedCycle); });
