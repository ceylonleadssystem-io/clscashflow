(function() {
  ...
  window.clsBuildInvoicePrintHtml = function(opts) {
    ...
    if (!note || note === DEFAULT_INVOICE_FOOTER) note = 'Thank you for your business.';
    var title = inv.num || inv.id || 'PREVIEW';
    var logoWidth = s.logoSize === 'L' ? 34 : (s.logoSize === 'S' ? 18 : 26);
    var logoStyle = 'max-width:' + logoWidth + 'mm;max-height:18mm';
    var logo = s.logo
    ...
    var css = '@page{size:A4;margin:0}' +
      ...
      '.brand-rule{height:4px;background:var(--accent);margin-bottom:14mm}.invoice-head{display:grid;grid-template-columns:minmax(0,1fr) 64mm;gap:12mm;align-items:start;margin-bottom:13mm}.brand{text-align:' + (s.logoAlign === 'right' ? 'right' : (s.logoAlign === 'center' ? 'center' : 'left')) + '}.logo-img{display:inline-block;object-fit:contain;margin-bottom:5mm}.logo-box{display:inline-flex;width:28mm;height:16mm;border:1px dashed var(--line);align-items:center;justify-content:center;font-size:9px;letter-spacing:3px;color:#9b9188;margin-bottom:5mm}.biz-name{font-family:' + titleFont + ';font-size:24px;line-height:1.15;font-weight:800;color:#111}.muted{font-size:10.5px;line-height:1.55;color:#6e635a;white-space:pre-line;word-break:break-word}.invoice-title{text-align:right}.invoice-title h1{font-family:' + titleFont + ';font-size:36px;line-height:1;text-transform:uppercase;letter-spacing:5px;color:#111}.invoice-title .num{margin-top:4mm;font-size:12px;font-weight:800;letter-spacing:.04em}.invoice-title .meta{margin-top:5mm;font-size:10.5px;line-height:1.7;color:#6e635a}.parties{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr) 32mm;gap:10mm;border-top:1px solid var(--line);border-bottom:1px solid var(--line);padding:8mm 0;margin-bottom:10mm}.label{font-size:8.5px;letter-spacing:2.5px;text-transform:uppercase;font-weight:800;color:var(--dark);margin-bottom:3mm}.party-name{font-size:14px;font-weight:800;line-height:1.25;margin-bottom:2mm}.status-pill{display:inline-block;border:1px solid var(--line);border-radius:999px;padding:4px 9px;font-size:10px;font-weight:800;text-transform:capitalize;margin-bottom:3mm}.items{width:100%;border-collapse:collapse;table-layout:fixed;margin-bottom:9mm}.items th{background:var(--dark);color:#fff;font-size:8.5px;letter-spacing:2px;text-transform:uppercase;text-align:left;padding:8px 10px}.items th:nth-child(1){width:49%}.items th:nth-child(2){width:10%}.items th:nth-child(3){width:20%}.items th:nth-child(4){width:21%}.items th:nth-child(n+2),.items td:nth-child(n+2){text-align:right}.items td{border-bottom:1px solid var(--line);padding:9px 10px;font-size:10.5px;line-height:1.35;vertical-align:top;word-break:break-word}.line-no{display:inline-block;min-width:18px;margin-right:7px;font-weight:800;color:var(--accent)}.line-desc{font-weight:650}.money-row{display:flex;justify-content:space-between;gap:10mm;border-bottom:1px solid var(--line);padding:6px 0;font-size:10.5px;color:#594f47}.money-row b{color:#111;font-variant-numeric:tabular-nums}.invoice-bottom{display:grid;grid-template-columns:minmax(0,1fr) 74mm;gap:14mm;align-items:start;margin-top:auto;padding-top:5mm}.notes{border-left:4px solid var(--accent);padding-left:5mm;min-height:28mm}.note-text{font-size:10.5px;line-height:1.55;color:#6e635a;white-space:pre-line}.totals{border-top:1px solid var(--line);padding-top:2mm}.grand{margin-top:4mm;background:var(--dark);color:#fff!important;border:0;padding:11px 13px;align-items:center}.grand span{font-size:8.5px;letter-spacing:2.8px;text-transform:uppercase;font-weight:800}.grand b{font-size:17px;color:#fff}.powered{border-top:1px solid var(--line);margin-top:8mm;padding-top:4mm;text-align:center;font-size:9px;letter-spacing:.04em;color:#7d736a}.powered b{color:#18130f}.view-classic .invoice-title h1,.view-olden .invoice-title h1{text-transform:none;letter-spacing:0}.view-olden{--paper:#fbf4e8}.view-olden .brand-rule{height:6px;background:transparent;border-top:3px double var(--accent);border-bottom:1px solid var(--accent)}.view-minimal{--paper:#fff}.view-minimal .brand-rule,.view-minimal .items th,.view-minimal .grand{background:#111}.view-bold .invoice-head{background:var(--dark);color:#fff;padding:9mm;margin:-2mm 0 12mm}.view-bold .invoice-title h1,.view-bold .invoice-title .num,.view-bold .biz-name{color:#fff}.view-bold .muted,.view-bold .invoice-title .meta{color:rgba(255,255,255,.78)}.tpl-pop .items th,.tpl-pop .grand{background:var(--accent);color:#1b1713}.tpl-green .items th,.tpl-yellow .items th{background:var(--accent);color:#111}@media print{html,body{background:#fff}.invoice-page{box-shadow:none;margin:0}}';
    return '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>' + invoiceEscape(title) + '</title><style>' + css + '</style></head><body>' +
      ...
      '<section class="parties"><div><div class="label">From</div><div class="party-name">' + invoiceEscape(s.biz) + '</div><div class="muted">' + invoiceBreaks(s.addr) + (s.email ? '<br>' + invoiceEscape(s.email) : '') + '</div></div><div><div class="label">Bill To</div><div class="party-name">' + invoiceEscape(inv.client || 'Customer') + '</div><div class="muted">' + invoiceBreaks(inv.caddr || '') + (inv.cemail ? '<br>' + invoiceEscape(inv.cemail) : '') + (inv.cphone ? '<br>' + invoiceEscape(inv.cphone) : '') + '</div></div><div><div class="label">Status</div><div class="status-pill">' + invoiceEscape(status) + '</div><div class="muted">Balance due<br><b>' + invoiceMoney(cur, balance) + '</b></div></div></section>' +
      '<table class="items"><thead><tr><th>Description</th><th>Qty</th><th>Unit price</th><th>Amount</th></tr></thead><tbody>' + rows + '</tbody></table>' +
