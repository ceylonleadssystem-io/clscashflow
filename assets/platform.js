(function() {
  ...
  function bindBillingCyclePicker(root,profile,details,initialCycle) {
    ...
    function select(nextCycle) {
      ...
      if(copy)copy.innerHTML=root.id==='cls-paywall'?(paidBefore?'Your current paid '+unit+' has ended.':'Your free trial has ended.')+' Transfer <b>'+escapeHtml(money(amount))+'</b> in advance for the next '+unit+', then upload the payment slip. Your data remains safe and the system resumes immediately after the slip is submitted.':'Transfer <b>'+escapeHtml(money(amount))+'</b> for your '+escapeHtml(details.name)+' plan, then upload the payment slip. If you pay during the trial, the paid '+unit+' begins when the 15-day trial ends.';
      if(timeline)timeline.outerHTML=paymentTimelineHtml(profile,cycle);if(submit)submit.textContent='Upload slip and activate '+cycle+' access';if(summary)summary.innerHTML='Selected: <b>'+cycle.charAt(0).toUpperCase()+cycle.slice(1)+'</b> · Transfer <b>'+escapeHtml(money(amount))+'</b>.';
    }
  ...
  window.clsOpenBankTransferPayment=function clsOpenBankTransferPayment(plan,profile,requestedCycle){
    ...
    profile=profile||window._profile||{};plan=bestPlan(profile,plan);var details=PLAN_DETAILS[plan]||PLAN_DETAILS.solo,cycle=billingCycleFor(profile,requestedCycle),annual=cycle==='annual',chargeAmount=annual?details.price:details.monthlyPrice,unit=annual?'year':'month',old=document.getElementById('cls-bank-payment-modal');if(old)old.remove();var wrap=document.createElement('div');wrap.id='cls-bank-payment-modal';wrap.className='cls-bank-modal';wrap.innerHTML='<div class="cls-bank-card"><button class="cls-bank-close" aria-label="Close">×</button><div class="cls-billing-kicker">Prepaid '+cycle+' subscription</div><h2>Pay your next '+unit+' in advance</h2><p class="cls-payment-copy">Transfer <b>'+escapeHtml(money(chargeAmount))+'</b> for your '+escapeHtml(details.name)+' plan, then upload the payment slip. If you pay during the trial, the paid '+unit+' begins when the 15-day trial ends.</p>'+billingCyclePickerHtml(details,cycle)+bankDetailsHtml()+paymentTimelineHtml(profile,cycle)+'<label class="cls-receipt-upload">Payment slip<input type="file" accept="application/pdf,image/png,image/jpeg,image/webp" required></label><button class="cls-bank-submit">Upload slip and activate '+cycle+' access</button><div class="cls-bank-status">The attachment will be emailed securely to '+escapeHtml(CLS_BANK.email)+'.</div></div>';document.body.appendChild(wrap);var getCycle=bindBillingCyclePicker(wrap,profile,details,cycle);wrap.querySelector('.cls-bank-close').onclick=function(){wrap.remove();};wrap.addEventListener('click',function(e){if(e.target===wrap)wrap.remove();});wrap.querySelector('.cls-bank-submit').onclick=async function(){var btn=this,status=wrap.querySelector('.cls-bank-status'),file=wrap.querySelector('input').files[0],selectedCycle=getCycle();try{btn.disabled=true;await window.clsSubmitSubscriptionReceipt(file,profile,plan,status,selectedCycle);btn.textContent='Thank you — '+selectedCycle+' access activated';setTimeout(function(){wrap.remove();},1300);}catch(e){status.textContent=e.message||'Upload failed.';btn.disabled=false;}};
  };
  window.clsStartCurrentPlanPayment=function(){return window.clsOpenBankTransferPayment(null,window._profile||{});};
  window.clsRenderSubscriptionPaywall=function(profile,opts){opts=opts||{};if(document.getElementById('cls-paywall'))return;injectBillingCardStyle();profile=profile||{};var plan=bestPlan(profile,opts.plan),details=PLAN_DETAILS[plan]||PLAN_DETAILS.solo,cycle=billingCycleFor(profile),annual=cycle==='annual',chargeAmount=annual?details.price:details.monthlyPrice,unit=annual?'year':'month',hasPaidBefore=!!(profile.lastPaymentSlipAt||profile.lastPaymentSlipAtUtc||profile.nextPaymentDue),ov=document.createElement('div');ov.id='cls-paywall';ov.className='cls-bank-modal';ov.setAttribute('data-paid-before',String(hasPaidBefore));ov.innerHTML='<div class="cls-bank-card cls-bank-paused"><div class="cls-billing-kicker">'+(hasPaidBefore?(annual?'Annual renewal due':'Monthly payment due'):'15-day trial complete')+'</div><h2>'+(hasPaidBefore?'Pay the next '+unit+' to continue':'Start your prepaid '+cycle+' plan')+'</h2><p class="cls-payment-copy">'+(hasPaidBefore?'Your current paid '+unit+' has ended.':'Your free trial has ended.')+' Transfer <b>'+escapeHtml(money(chargeAmount))+'</b> in advance for the next '+unit+', then upload the payment slip. Your data remains safe and the system resumes immediately after the slip is submitted.</p>'+billingCyclePickerHtml(details,cycle)+bankDetailsHtml()+paymentTimelineHtml(profile,cycle)+'<label class="cls-receipt-upload">Payment slip<input type="file" accept="application/pdf,image/png,image/jpeg,image/webp" required></label><button class="cls-bank-submit">Upload slip and activate '+cycle+' access</button><div class="cls-bank-status">PDF, PNG, JPG or WebP, up to 3 MB. The slip will be emailed to '+escapeHtml(CLS_BANK.email)+'.</div><button class="cls-bank-signout" onclick="window.clsSignOut&&window.clsSignOut()">Sign out</button></div>';document.body.appendChild(ov);var getCycle=bindBillingCyclePicker(ov,profile,details,cycle);ov.querySelector('.cls-bank-submit').onclick=async function(){var btn=this,status=ov.querySelector('.cls-bank-status'),file=ov.querySelector('input').files[0],selectedCycle=getCycle();try{btn.disabled=true;await window.clsSubmitSubscriptionReceipt(file,profile,plan,status,selectedCycle);btn.textContent='Payment submitted — '+selectedCycle+' access restored';}catch(e){status.textContent=e.message||'Upload failed.';btn.disabled=false;}};};
  ...
  function mountBillingWidget() {
    ...
    injectBillingCardStyle();
	    ...
	    function renderCycle() {
	      ...
	      wrap.querySelector('[data-billing-copy]').textContent = 'Pay your CLS ' + details.name + ' ' + selectedCycle + ' package directly to our Commercial Bank account. Upload the payment slip after transfer so it reaches our accounts team.';
