...
exports.handler = async function handler(event) {
  ...
  const rawLabel = String(d.documentLabel || 'Invoice').trim();
  const documentLabel = /^receipt$/i.test(rawLabel) ? 'Receipt' : (/^estimate$/i.test(rawLabel) ? 'Estimate' : (/^quote|quotation$/i.test(rawLabel) ? 'Quote' : 'Invoice'));
  const documentNumber = esc(d.documentNumber || d.invNum || '');
  ...
  const sub = d.sub != null ? Number(d.sub || 0) : lines.reduce(function(total, line) {
    return total + (line.total != null ? Number(line.total || 0) : Number(line.qty || 1) * Number(line.price || line.rate || 0));
  }, 0);
  const disc = d.disc || 0;
  ...
  +     '</td>'
  +     '<td style="vertical-align:top;text-align:right"><div style="font-size:26px;font-weight:700;letter-spacing:3px">' + esc(documentLabel.toUpperCase()) + '</div>'
  +       '<div style="font-size:13px;opacity:.9;margin-top:4px">' + documentNumber + '</div></td>'
  +   '</tr></table>'
  + '</div>'
  // intro / reminder
  + '<div style="padding:28px 32px 8px">'
  +   '<p style="font-size:15px;color:#15110d;margin:0 0 14px">Dear ' + esc(d.clientName || 'Customer') + ',</p>'
  +   '<p style="font-size:14px;color:#4a463f;line-height:1.7;margin:0 0 6px">Please find your ' + esc(documentLabel.toLowerCase()) + ' from <strong>' + biz + '</strong> below. '
  +   (documentLabel === 'Invoice' ? 'Kindly arrange payment by the due date shown. If you have already paid, please disregard this reminder.' : (documentLabel === 'Receipt' ? 'Your payment has been received successfully.' : 'Please review the details before the validity date shown.')) + '</p>'
  + '</div>'
  // meta
  ...
  +       '<td style="padding:3px 0;text-align:right">' + (documentLabel === 'Invoice' ? 'Due Date' : (documentLabel === 'Receipt' ? 'Payment Date' : 'Valid Until')) + ': <strong style="color:#15110d">' + esc(d.due || d.date || '') + '</strong></td></tr>'
  ...
  +       '<td style="padding:14px 16px;color:#ffffff;font-size:12px;letter-spacing:1px">' + (documentLabel === 'Invoice' ? 'TOTAL DUE' : (documentLabel === 'Receipt' ? 'TOTAL PAID' : 'TOTAL')) + '</td>'
  ...
  +   esc(documentLabel) + ' generated by Cashflow System - <strong style="color:' + accent + '">CeylonryLabs.io</strong></div>'
  + '</div></div>';

  const textBody =
    'Dear ' + (d.clientName || 'Customer') + ',\n\n'
    + 'Please find your ' + documentLabel.toLowerCase() + ' from ' + (d.bizName || 'Your Business') + '.\n\n'
    + documentLabel + ': ' + (d.documentNumber || d.invNum || '') + '\n'
    + 'Date: ' + (d.date || '') + '\n'
    + (documentLabel === 'Invoice' ? 'Due' : (documentLabel === 'Receipt' ? 'Payment Date' : 'Valid Until')) + ': ' + (d.due || d.date || '') + '\n'
    + (documentLabel === 'Invoice' ? 'Amount Due' : (documentLabel === 'Receipt' ? 'Total Paid' : 'Total')) + ': ' + (d.cur || 'LKR') + ' ' + money(d.amount) + '\n\n'
    + (d.notes || 'Thank you for your business.') + '\n\n'
    + documentLabel + ' generated by Cashflow System - CeylonryLabs.io';
  ...
  try {
    await transporter.sendMail({
      ...
      subject: documentLabel + ' ' + (d.documentNumber || d.invNum || '') + ' from ' + (d.bizName || 'Your Business') + ' — ' + (d.cur || 'LKR') + ' ' + money(d.amount),
      text: textBody,
      html: html
    });
