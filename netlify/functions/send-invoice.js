...
exports.handler = async function handler(event) {
  ...
  const documentLabel = /^estimate$/i.test(rawLabel) ? 'Estimate' : (/^quote|quotation$/i.test(rawLabel) ? 'Quote' : 'Invoice');
  ...
  +   (documentLabel === 'Invoice' ? 'Kindly arrange payment by the due date shown. If you have already paid, please disregard this reminder.' : 'Please review the details before the validity date shown.') + '</p>'
  + '</div>'
  // meta
  ...
  +       '<td style="padding:3px 0;text-align:right">' + (documentLabel === 'Invoice' ? 'Due Date' : 'Valid Until') + ': <strong style="color:#15110d">' + esc(d.due || '') + '</strong></td></tr>'
  ...
  +       '<td style="padding:14px 16px;color:#ffffff;font-size:12px;letter-spacing:1px">' + (documentLabel === 'Invoice' ? 'TOTAL DUE' : 'TOTAL') + '</td>'
  ...
  const textBody =
    ...
    + 'Date: ' + (d.date || '') + '\n'
    + (documentLabel === 'Invoice' ? 'Due' : 'Valid Until') + ': ' + (d.due || '') + '\n'
    + (documentLabel === 'Invoice' ? 'Amount Due' : 'Total') + ': ' + (d.cur || 'LKR') + ' ' + money(d.amount) + '\n\n'
