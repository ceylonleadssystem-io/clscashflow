...
test('cloud merge preserves catalogue, category and inventory deletions', () => {
  assert.match(html, /deletedIds:\{products:\[\],modifiers:\[\],inventory:\[\],sales:\[\],saleReceipts:\[\],users:\[\],categories:\[\],subcategories:\[\]\}/);
...
test('an empty sync cannot silently erase an existing catalogue', () => {
  ...
  assert.match(html, /if\(localPayload&&typeof localPayload==='object'&&!localPayload\.accountUid\)localPayload\.accountUid=workspaceUid/);
...
test('sync saves offline first and cannot remain stuck indefinitely', () => {
  ...
  assert.match(html, /cloudUnsubscribe=setInterval\(pull,1500\)/);
